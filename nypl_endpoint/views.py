...
def index(request):
    ...
    return render(request, 'nypl_endpoint/index.html', context)
...
class RandomItemView(View):
    ...
    def get(self, request):
        ...
        # a JSONDecode error here likely means that the authorization headers were not supplied
        try:
            data = captures.json()
        except:
            return HttpResponse("Check that you are authorized to access the API.")
        ...
        # if we didn't find a typeOfResource key that matched, let the user know
            ...
            return HttpResponse("Looks like we haven't added anything of that type recently!")
